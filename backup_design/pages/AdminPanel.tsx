...
const AdminPanel: React.FC = () => {
  ...
  const handleMainSettingsSave = async () => {
    ...
    const { appName, footerText, navLinks, logoUrl, logoPath, siteUrl, whatsappNumber } =
      localSettings;
  ...
  const handleResetToDefaults = async () => {
    if (
      !confirm(
        '⚠️ Questo ripristinerà tutte le configurazioni ai valori di default e nasconderà Admin dalla navbar pubblica. Continuare?'
      )
    ) {
      return;
    }
      ...
      <div className="bg-gray-800/50 p-6 rounded-lg shadow-lg border border-gray-700/50">
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              {localSettings.logoUrl && (
                <img
                  ...
                  alt="logo"
                  className="h-10 w-auto bg-gray-700 rounded"
                />
              )}
              <input
                ref={logoInputRef}
                ...
                className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-teal-600 file:text-white hover:file:bg-teal-700 transition-colors"
              />
              {localSettings.logoUrl && (
                <button
          ...
          <div>
            ...
            <p className="text-xs text-teal-400 mt-1">Esempio: https://gallery.tuodominio.it</p>
          </div>

          <div>
            <label htmlFor="whatsappNumber" className="block text-sm font-medium text-gray-300 mb-1">
              📱 WhatsApp (numero internazionale senza +)
            </label>
            <input
              id="whatsappNumber"
              name="whatsappNumber"
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Questo numero verrà usato sulla landing per indirizzare le richieste direttamente su WhatsApp.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {localSettings.navLinks.map((link, index) => (
                <div
                  key={link.id}
                  className="flex items-center gap-2 p-2 bg-gray-900/50 rounded-md"
                >
                  <input
                    type="text"
      ...
      <div className="bg-gray-800/50 p-6 rounded-lg shadow-lg border border-gray-700/50">
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="flex items-center gap-2 p-3 bg-amber-900/20 border border-amber-600/30 rounded-md">
            ...
            <div className="flex-grow">
              ...
              <div className="text-xs text-amber-300/80">
                Recupera tutte le foto da Firebase Storage
              </div>
            </div>
            <button
          ...
          <div className="flex items-center gap-2 p-3 bg-red-900/20 border border-red-600/30 rounded-md">
            ...
            <div className="flex-grow">
              ...
              <div className="text-xs text-red-300/80">
                Ripristina le configurazioni di default (nasconde Admin dalla navbar)
              </div>
            </div>
            <button
          ...
          <div className="mt-6">
            ...
            <BackupManager
              onBackupCreated={() => logger.info('✅ Backup creato con successo!')}
              onBackupRestored={() => logger.info('✅ Configurazione ripristinata!')}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          {albums.map((album) => (
            <div key={album.id} className="bg-gray-900/70 p-4 rounded-md border border-gray-700">
              ...
              {expandedAlbumId === album.id && <AlbumPhotoManager key={album.id} album={album} />}
            </div>
          ))}
        </div>
      </div>
            ...
            <div className="mb-4">
              <label className="flex items-center cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setLocalSettings((prev) => ({ ...prev, aiEnabled: e.target.checked }))
                  }
                  className="w-4 h-4 text-teal-600 bg-gray-900 border-gray-700 rounded focus:ring-teal-500"
                />
                <span className="ml-2 text-sm font-medium text-gray-300">

...
const GtmScript: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (currentGtmId.current && currentGtmId.current !== gtmId) {
      logger.info(`GTM: Cleaning up previous installation (${currentGtmId.current})`);
      ...
      if (window.dataLayer) {
        logger.info('GTM: Clearing dataLayer for new container');
    ...
    if (gtmId && consentStatus === 'granted') {
      logger.info(`GTM: Installing container ${gtmId}`);
      ...
      logger.info('GTM: Removing container (ID cleared)');

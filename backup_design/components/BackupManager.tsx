...
const BackupManager: React.FC<BackupManagerProps> = ({ onBackupCreated, onBackupRestored }) => {
  ...
  const loadCloudBackups = async () => {
    ...
    try {
      ...
      logger.error('Error loading cloud backups:', error);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        {cloudBackups.length > 0 ? (
          <div className="space-y-2 max-h-40 overflow-y-auto">
            {cloudBackups.slice(0, 5).map((backup) => (
              <div
                key={backup.name}
                className="flex items-center justify-between bg-blue-800/30 p-2 rounded text-sm"
              >
                <div>

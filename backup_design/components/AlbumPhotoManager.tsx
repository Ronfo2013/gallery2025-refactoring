...
const AlbumPhotoManager: React.FC<AlbumPhotoManagerProps> = ({ album, brandId }) => {
  ...
  const handleUploadAll = async () => {
    ...
    logger.info(`🚀 Starting parallel upload of ${filesToProcess.length} files...`);
    ...
    try {
      ...
      if (uploadedPhotos.length > 0) {
        logger.info(`💾 Saving batch of ${uploadedPhotos.length} photos to Firestore...`);
                  ...
                  <img
                    ...
                    onError={(e) => {
                      ...
                      if (photo.thumbUrl && target.src.includes('_thumb_200')) {
                        logger.warn(`⚠️ Thumbnail failed for ${photo.id}, trying optimized WebP`);

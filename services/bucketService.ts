...
const generateInitialData = (): { albums: Album[]; siteSettings: SiteSettings } => {
  return {
    ...
    siteSettings: {
      ...
      navLinks: [{ id: 'nav-1', text: 'Gallery', to: '/' }],
      gtmId: '',
      siteUrl: '', // Custom site URL for sharing (empty = use Cloud Run URL)
      whatsappNumber: '', // Numero WhatsApp per contatto diretto
      seo: {
...
const performMigration = (config: any): AppConfig => {
  ...
  if (!config.siteSettings) {
    ...
    config.siteSettings.logoUrl =
      config.siteSettings.logoUrl !== undefined
        ? config.siteSettings.logoUrl
        : defaults.siteSettings.logoUrl;
    config.siteSettings.footerText =
      config.siteSettings.footerText || defaults.siteSettings.footerText;
    ...
    config.siteSettings.whatsappNumber =
      config.siteSettings.whatsappNumber !== undefined
        ? config.siteSettings.whatsappNumber
        : defaults.siteSettings.whatsappNumber;

    // New AI-related fields (added later, need migration)
    config.siteSettings.aiEnabled =
      config.siteSettings.aiEnabled !== undefined
        ? config.siteSettings.aiEnabled
        : defaults.siteSettings.aiEnabled;
    config.siteSettings.geminiApiKey =
      config.siteSettings.geminiApiKey || defaults.siteSettings.geminiApiKey;

    // New URL and preloader fields (migration)
    config.siteSettings.siteUrl =
      config.siteSettings.siteUrl !== undefined
        ? config.siteSettings.siteUrl
        : defaults.siteSettings.siteUrl;
    config.siteSettings.preloader =
      config.siteSettings.preloader || defaults.siteSettings.preloader;
  }
...
export const getConfig = async (): Promise<AppConfig> => {
  try {
    ...
    logger.error('Error getting config from Firestore:', error);
...
export const saveConfig = async (config: AppConfig): Promise<void> => {
  try {
    ...
    logger.info('Config saved to Firestore successfully');
    ...
    if ((error as any).code === 'permission-denied') {
      // ℹ️ This is expected for non-admin users who don't have write access to global config
      logger.info('ℹ️ Config update skipped: non-admin user (permission denied)');
    } else {
      logger.error('Error saving config to Firestore:', error);
    }
    throw error;
  }
};
...
export const uploadFile = async (
  ...
  try {
    ...
    logger.info(
      `⬆️ Uploading ${file.name} (${(file.size / 1024 / 1024).toFixed(2)}MB) for brand ${brandId}...`
    );
...
export const deleteFile = async (path: string): Promise<void> => {
  try {
    ...
    logger.info(`🗑️ Deleted original: ${path}`);
    ...
    const deletePromises = processedFiles.map(async (filePath) => {
      try {
        ...
        if ((error as any).code !== 'storage/object-not-found') {
          logger.warn(`⚠️ Could not delete ${filePath}:`, error);
        }
      }
    });

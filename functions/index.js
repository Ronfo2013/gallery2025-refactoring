/**
 * Cloud Functions for AI Photo Gallery - Multi-Brand MVP (Gen2)
 *
 * Includes:
...
// ============================================
// 🏢 BRAND MANAGEMENT FUNCTIONS (NEW)
// ============================================
const { createBrand } = require('./lib/createBrand');
 ...
 */
exports.deleteThumbnails = onObjectDeleted(async (event) => {
  const object = event.data;
  const filePath = object.name;
  const fileName = path.basename(filePath);
  const fileDir = path.dirname(filePath);
  ...
  console.log('🗑️ Original deleted, cleaning up processed versions:', filePath);

  const bucket = admin.storage().bucket(object.bucket);
  const filesToDelete = ['_optimized.webp', '_thumb_200.webp', '_thumb_800.webp'];

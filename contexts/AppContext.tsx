...
interface AppContextType {
  ...
  addPhotoToAlbum: (
    ...
    skipSave?: boolean,
    brandId?: string
  ) => Promise<{ photo: any; albums: Album[] } | void>;
  uploadPhotoOnly: (photoFile: File, title: string, brandId?: string) => Promise<Photo>;
  updatePhotoUrls: (
    photoId: string,
    urls: { optimizedUrl?: string; thumbUrl?: string; mediumUrl?: string; needsWebPRetry?: boolean }
  ) => void;
  checkWebPGeneration: (
    photo: Photo
  ) => Promise<{ optimizedUrl?: string; thumbUrl?: string; mediumUrl?: string }>;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const hasAdminInNav = config.siteSettings.navLinks?.some(
  ...
  useEffect(() => {
    ...
    if (now - lastSaveTime < 1000) {
      return;
    } // Debounce saves (max 1 per second)
  ...
  useEffect(() => {
    ...
    const retryWebPForFailedPhotos = async () => {
      ...
      if (photosNeedingRetry.length === 0) {
        return;
      }
  ...
  const addPhotoToAlbum = async (
    ...
    const { path, url, optimizedUrl, thumbUrl, mediumUrl } = await bucketService.uploadFile(
      photoFile,
      brand?.id || ''
    );
    ...
    const uniqueId = generatePhotoId(photoIdCounterRef.current);
        ...
        if (album.id === albumId) {
          ...
          if (updatedPhotos.length === 1) {
            return {
              ...
              coverPhotoUrl:
                newPhoto.optimizedUrl || newPhoto.mediumUrl || newPhoto.thumbUrl || newPhoto.url,
            };
          }
  ...
  const updatePhotoUrls = (
    ...
    setAlbums((prevAlbums) => {
      return prevAlbums.map((album) => ({
        ...album,
        photos: album.photos.map((photo) => (photo.id === photoId ? { ...photo, ...urls } : photo)),
      }));
    });
  };

  // 🔍 Check if WebP versions have been generated for a photo
  const checkWebPGeneration = async (
    photo: Photo
  ): Promise<{ optimizedUrl?: string; thumbUrl?: string; mediumUrl?: string }> => {
    if (!photo.path) {
      return {};
    ...
    setAlbums((prevAlbums) => {
      const updatedAlbums = prevAlbums.map((album) => {
        if (album.id === albumId) {
          ...
          const coverWasDeleted = album.photos.find(
            (p) =>
              (p.optimizedUrl === album.coverPhotoUrl ||
                p.mediumUrl === album.coverPhotoUrl ||
                p.thumbUrl === album.coverPhotoUrl ||
                p.url === album.coverPhotoUrl) &&
              idSet.has(p.id)
          );
          ...
          if (coverWasDeleted) {
            ...
            newCoverUrl =
              remainingPhotos.length > 0
                ? remainingPhotos[0].optimizedUrl ||
                  remainingPhotos[0].mediumUrl ||
                  remainingPhotos[0].thumbUrl ||
    ...
    setAlbums((prevAlbums) => {
      const updatedAlbums = prevAlbums.map((album) => {
        if (album.id === albumId) {
          ...
          const newCoverUrl =
            newPhotos.length > 0
              ? newPhotos[0].optimizedUrl ||
                newPhotos[0].mediumUrl ||
                newPhotos[0].thumbUrl ||
                newPhotos[0].url
              : 'https://placehold.co/800x600/1a202c/4a5568?text=No+Photos';
  ...
  const updateSiteSettings = async (newSettings: Partial<SiteSettings>, newLogoFile?: File) => {
    ...
    if (newLogoFile) {
      ...
      const { path, url } = await bucketService.uploadFile(newLogoFile, brand?.id || '');
  ...
  const getSeoSuggestions = async (): Promise<SeoSettings> => {
    ...
    return await generateSeoSuggestions(
  ...
  const searchPhotos = async (albumId: string, query: string): Promise<string[]> => {
    ...
    const photoInfo = album.photos.map((p) => ({
      id: p.id,
      title: p.title,
  ...
  const recoverFromStorage = async () => {
    try {
      ...
      const recoveryAlbum: Album = {
        ...
        coverPhotoUrl:
          recoveredPhotos[0]?.optimizedUrl ||
          recoveredPhotos[0]?.thumbUrl ||
          recoveredPhotos[0]?.url ||
          'https://placehold.co/800x600/1a202c/4a5568?text=No+Photos',

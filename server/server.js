...
// Serve static files from dist ONLY (not from root)
app.use(express.static(staticPath, { index: false }));

// SPA fallback - serve index.html only for non-file routes
app.get('*', (req, res, next) => {
    // If the request is for a file (has an extension), let it 404 naturally
    if (path.extname(req.path) !== '') {
        return next();
    }
    // Otherwise, serve index.html for SPA routing
    const indexPath = path.join(staticPath, 'index.html');
    console.log(`[SPA Fallback] Serving: ${indexPath} for route: ${req.path}`);
    console.log(`[SPA Fallback] staticPath: ${staticPath}`);
    console.log(`[SPA Fallback] __dirname: ${__dirname}`);
